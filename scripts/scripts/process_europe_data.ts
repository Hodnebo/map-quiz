...
function processEuropeData() {
  const inputPath = path.join(__dirname, '../../web/public/data/europe.json');
  const outputPath = path.join(__dirname, '../../web/public/data/europe_simplified.geo.json');

  console.log('Reading europe.json...');
  const rawData: EuropeGeoJSON = JSON.parse(fs.readFileSync(inputPath, 'utf8'));
